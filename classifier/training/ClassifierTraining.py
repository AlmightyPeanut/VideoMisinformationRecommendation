...
class ClassifierTraining(object):
    ...
    """
    def __init__(self, gpu_training=False):
        """
        Constructor
    ...
    def train_model(self):
        ...
        for train_val_set_indices, test_set_indices in stratified_kfold.split(X=self.dataset_videos, y=self.dataset_labels_categorical):
            ...
            if Config.CLASSIFICATION_THRESHOLD is not None:
                ...
                for predicted_probas in test_pred_proba:
                    ...
                    elif predicted_probas[0] >= Config.CLASSIFICATION_THRESHOLD:
                        test_predicted_classes.append(0)
                    else:
                        test_predicted_classes.append(2)
            else:
        ...
        pickle.dump([mean_fpr, mean_tpr], file=open('{0}/roc_metrics.p'.format(self.TRAINING_MODELS_BASE_DIR), mode='wb'))
        ...
        final_model_metrics = {
            "accuracy": folds_performance_metrics[best_kfold - 1][0],
            "precision": folds_performance_metrics[best_kfold - 1][1],
            "recall": folds_performance_metrics[best_kfold - 1][2],
            "f1_score": folds_performance_metrics[best_kfold - 1][3],
            "confusion_matrix": folds_confusion_matrices[best_kfold - 1].tolist()
        }
        with open('{0}/pseudoscience_model_final_metrics.json'.format(self.BEST_MODEL_BASE_DIR), 'w') as f:
        ...
        copyfile(src='{0}/K={1}_pseudoscience_model.hdf5'.format(self.TRAINING_MODELS_BASE_DIR, best_kfold),
                 dst='{0}/pseudoscience_model_final.hdf5'.format(self.BEST_MODEL_BASE_DIR))
        return
