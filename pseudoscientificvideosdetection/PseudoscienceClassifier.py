...
class PseudoscienceClassifier(object):
    ...
    def classify(self, video_details):
        ...
        predicted_proba = self.PSEUDOSCIENCE_CLASSIFIER.predict(classifier_input, batch_size=1, verbose=0)

        # Decode Predicted probability and convert it to a Label
        if self.CLASSIFICATION_THRESHOLD is not None:

            debunking_probability = np.round(predicted_proba.item(0), decimals=3)
            ...
            elif Decimal(debunking_probability) >= Decimal(self.CLASSIFICATION_THRESHOLD):
                predicted_class = self.CLASSES[0]
            else:
                predicted_class = self.CLASSES[2]
        else:
        ...
        neutral_proba = np.round(predicted_proba.item(2), decimals=3)
        ...
        if predicted_class == 'debunking':
            conf_score = science_proba
        elif predicted_class == 'promoting':
            conf_score = pseudoscience_proba
        else:
            conf_score = neutral_proba
